...
type AdminTab = 'reviews' | 'services' | 'bio' | 'gallery' | 'brands' | 'bookings' | 'contact' | 'domain' | 'settings';
...
export default function AdminShell({
  ...
  const nav: { key: AdminTab; label: string; href: string }[] = [
    ...
    { key: 'settings', label: t.navSettings, href: '/admin/settings' },
  ];
  ...
  return (
    <div className="min-h-screen bg-background px-6 py-10">
      <div className={`mx-auto ${maxWidth}`}>
        <div className="mb-8">
          {/* Title + actions on one row; the nav gets its own full-width row
              below so the tab list never competes with the buttons for space
              (it wrapped inconsistently between wide and narrow pages). */}
